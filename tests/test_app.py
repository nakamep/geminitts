...
def test_generate_audio_success(mock_generative_model, client):
    ...
    assert 'generation_config' in kwargs # Checks that it's passed
    # The actual generation_config object is now empty/default, so no response_modalities or speech_config within it.
    
    speech_config_arg = kwargs['speech_config'] # speech_config is now a direct kwarg
...
def test_generate_audio_specific_voice(mock_generative_model, client):
    ...
    args, kwargs = mock_model_instance.generate_content.call_args
    assert kwargs['contents'] == ["Hello from Puck"] 
    assert 'generation_config' in kwargs # Checks that it's passed
